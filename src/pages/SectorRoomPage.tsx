...
import { rewardForSector } from '../game3d/systems/gear'
import { getObjective } from '../game3d/story/objectives'
import { R3D, vec3, type LevelResult, type PuzzleResult, type Vec3 } from '../game3d/contracts'
...
function buildEnemies(order: number, size: [number, number]): EnemySpawn[] {
  const [w, d] = size
  const count = Math.min(2 + Math.floor(order / 1.5), 5)
  const speed = 2.4 + order * 0.2
  const hp = 3 + order
  // Later sectors add ranged shooters you have to dodge.
  const rangedCount = order < 2 ? 0 : order < 4 ? 1 : 2
  const slots: Vec3[] = [
    vec3(-w * 0.28, 1, -d * 0.1),
...
function RoomInner({ sectorId }: { sectorId: string }) {
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if ((k === 'e' || k === ' ' || k === 'enter') && !menuOpen && !puzzleOpen) {
        if (near === 'puzzle') {
          e.preventDefault()
          gs.setPaused(true)
          setPuzzleOpen(true)
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [near, menuOpen, puzzleOpen])

  const blocked = phase !== 'play' || puzzleOpen || menuOpen || invOpen || run.isGameOver

  // Quick inventory toggle with the I key.
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (e.key.toLowerCase() === 'i' && !menuOpen && !puzzleOpen && !run.isGameOver) {
        e.preventDefault()
        setInvOpen((o) => !o)
      }
    }
    window.addEventListener('keydown', onKey)
    return () => window.removeEventListener('keydown', onKey)
  }, [menuOpen, puzzleOpen, run.isGameOver])
  ...
  function handlePuzzleComplete(res: PuzzleResult) {
    ...
    // Reward gear for clearing the block.
    const reward = rewardForSector(sectorId)
    if (reward) {
      const added = inv.addItem(reward.id)
      setToast(added ? `Unlocked ${reward.icon} ${reward.name}` : `Already have ${reward.name}`)
    }
    if (uid) {
      ...
      {phase === 'results' && result && (
        <div className="puzzle-overlay">
          <ResultsScreen
            ...
            onRetry={resetRoom}
            onNext={nxt ? () => navigate(R3D.room(nxt.id)) : undefined}
